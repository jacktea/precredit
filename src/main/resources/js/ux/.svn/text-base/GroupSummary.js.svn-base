...
summaryRenderer: function(v, params, data){
    ...
    renderSummary : function(o, cs){
        ...
        for(var i = 0, len = cs.length; i < len; i++){
            ...
            if(cf.summaryType || cf.summaryRenderer){           	
            	cf.summaryRenderer = this.getDefaultSummaryRenderer(cf.summaryType,cf.summaryRenderer);            	
    ...
    getDefaultSummaryRenderer : function(summaryType,summaryRenderer){    	
    	if(summaryType){
    		if(summaryRenderer) return summaryRenderer;
    		switch (summaryType) {
			   case "count" :
			       return function(v, params, data){
		                return ((v === 0 || v >= 1) ? '(共' + v +' 条记录)' : '');
		            }
			   default :
			       return null;
			}
